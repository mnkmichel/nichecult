...
const { assignQrSampleSet, me } = useAuthApi()
...
onMounted(async () => {
  ...
  if (!token) {
    await navigateTo('/login?redirect=/qr-start')
    return
  }
  ...
  // Assign Sample Set 1 (or retrieve existing assignment)
  try {
    const res = await assignQrSampleSet(token)
  ...
  <main class="flex min-h-screen flex-col items-center justify-center bg-stone-100 px-4">
    <div class="w-full max-w-sm rounded-2xl border border-stone-200 bg-white p-8 text-center shadow-sm">
      <template v-if="status === 'loading'">
        ...
        <p class="text-sm text-stone-600">Wird vorbereitet…</p>
      </template>

      <template v-else-if="status === 'error'">
        <p class="text-sm font-semibold text-red-600">{{ errorMessage }}</p>
        <NuxtLink to="/" class="mt-5 inline-block text-sm text-stone-700 underline">Zurück zur Startseite</NuxtLink>
      </template>
    </div>
  </main>
</template>
